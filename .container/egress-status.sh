...
report() { # report <v4chain> <v6chain> <v6path> <allowlist> <daemon> <daemon_at>
    local v6
    v6="$(v6_from "$2" "$3")"
    printf 'state=%s\n' "$(probe_state "$1" "$2" "$3" "$4")"
    printf 'v4=%s\n'    "$1"
    ...
    printf 'daemon=%s\n' "$5"
    # WHERE that answer is about: this image's DAEMON_HOST:DAEMON_PORT. verify.sh runs the checkout's
    # library, which a pull without a rebuild can move; it probes this address, not its own.
    printf 'daemon_at=%s\n' "$6"
}
...
if [ "${1:-}" = "--self-test" ] && [ "$#" -eq 1 ]; then
    ...
    out="$(report bounded denied absent yes port h.example:1)"
    ...
    out="$(report open open open no absent h.example:1)"
    ...
    out="$(report bounded open absent no wide h.example:7117)"
    ...
    eq "...with the address it is about"  "$(printf '%s\n' "$out" | sed -n 's/^daemon_at=//p')" "h.example:7117"
...
report "$(v4_chain_state)" "$(v6_chain_state)" "$(v6_path_state)" "$(allowlist_state)" "$(daemon_state)" "$DAEMON_HOST:$DAEMON_PORT"
