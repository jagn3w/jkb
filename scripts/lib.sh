...
#
# Then it waits for `jkb serve` to prove it came up: the daemon writes its token (at `jkb service
# token-path`) only once its port is bound, so a token newer than a marker taken before activation is
