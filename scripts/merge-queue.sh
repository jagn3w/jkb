...
#
# ONCE, AT THE TOP, rather than `-C` on each of the thirteen call sites below: a rule every call
# site has to remember is the defect this repository keeps rediscovering, and a fourteenth git call
# added below would not have to remember this one. The six names are
# `gitrepo::REPO_SELECTION_VARS` — which repository, and which parts of one — kept in step with it
# by `dev-scripts.test.sh`'s case6, which reads the six names out of `REPO_SELECTION_VARS` itself.
...
# Measured on git 2.51.1: the hook fires on `merge --ff-only`, and `-c core.hooksPath=/dev/null`
# suppresses it while the merge still happens. The gate below has not run yet, so without this the
# queue installs a binary built from a candidate it may reject seconds later — and either way the
# queue is not the place to reinstall an operator's tooling mid-run. The same rule is stated at
# `gitrepo::graft`, the other implementation of this graft.
git switch "$BASE" >/dev/null 2>&1
